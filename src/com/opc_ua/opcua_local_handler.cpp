    ...
    for (auto iterRef = mNodesReferences.begin(); iterRef != mNodesReferences.end(); ++iterRef) {
      if (UA_NodeId_equal(iterRef->mNodeId, (*iterNode))) {
        ...
        while (stillSomethingThere) {
          ...
          for (auto itAction = iterRef->mActionsReferencingIt.begin(); itAction != iterRef->mActionsReferencingIt.end();
               ++itAction) {
        ...
        if (UA_STATUSCODE_GOOD == retVal) {
          ...
          if (UA_STATUSCODE_GOOD == retVal) {
            if (!handle) {
              ...
              DEVLOG_ERROR("[OPC UA LOCAL]: At FB %s RD_%d the node %s has already a FB who is reading from it. Cannot "
                           "add another one\n",
...
UA_StatusCode COPC_UA_Local_Handler::createVariableNode(const CCreateVariableInfo &paCreateVariableInfo) {
  ...
  if (paCreateVariableInfo.mParentNodeId) {
    ...
    DEVLOG_WARNING("[OPC UA LOCAL]: You are creating a variable %.*s in the /Root folder. This is not a good practice. "
  ...
  for (size_t i = 0; i < paCreateMethodInfo.mOutputSize + paCreateMethodInfo.mInputSize; i++) {
    ...
    if (i < paCreateMethodInfo.mOutputSize) {
      ...
      arg->name = UA_STRING_ALLOC(
          CStringDictionary::get(interfaceFB.mDINames[i + 2])); // we store the names of the SDs/RDs as names for the
      ...
      UA_Argument_init(arg);
      arg->name = UA_STRING_ALLOC(CStringDictionary::get(
          interfaceFB.mDONames[i - paCreateMethodInfo.mOutputSize + 2])); // + 2 skips the QO and STATUS
  ...
  for (auto it = paActionInfo.getNodePairInfo().begin(); it != paActionInfo.getNodePairInfo().end();
    ...
    if (UA_STATUSCODE_GOOD != retVal) {
      DEVLOG_ERROR("[OPC UA LOCAL]: Could not convert value to write for port %d at FB %s. Error: %s\n",
                   indexOfNodePair, paActionInfo.getLayer().getCommFB()->getInstanceName(), UA_StatusCode_name(retVal));
      break;
    }
  }
  return retVal;
...
UA_StatusCode COPC_UA_Local_Handler::createObjectNode(const CCreateObjectInfo &paCreateObjectInfo) const {
  ...
  if (paCreateObjectInfo.mParentNodeId) {
    ...
    parentNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_ROOTFOLDER);
    DEVLOG_WARNING("[OPC UA LOCAL]: You are creating an Object %.*s in the /Root folder. This is not a good practice. "
                   "Try to create them in the /Root/Objects folder\n",
  ...
  retVal = UA_Server_addObjectNode(mUaServer, requestedNodeId, parentNodeId,
                                   UA_NODEID_NUMERIC(0, UA_NS0ID_HASCOMPONENT), *paCreateObjectInfo.mBrowseName,
  ...
  if (-1 != retVal) {
    if (paBrowsePathsResults[foundFolderOffset + retVal].targetsSize == 0) {
      ...
      if (paBrowsePathsResults[foundFolderOffset + retVal].targetsSize > 1) {
        DEVLOG_WARNING(
            "[OPC UA LOCAL]: The given browse path has multiple results for the same path. Taking the first result.\n");
      }
    }
  }
  retVal++;
  ...
  if (!localMethodHandle) {
    ...
    if (paInputSize != localMethodHandle->getReceiveSize() || paOutputSize != localMethodHandle->getSendSize()) {
      ...
      if (e_ProcessDataOk ==
        ...
        // This function is called from the opcua server thread, so we release the lock so the method can be finished by
        // forte in executeAction there might be a problem here if another resource, which is not executing the method
        // response, uses the server and changes it somehow that it breaks something, like deleting the method maybe
